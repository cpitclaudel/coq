...
The name {\tacticdef{psatz}} is an abbreviation for \emph{positivstellensatz} -- literally positivity theorem -- which
...
\asection{\tacticdef{lra}: a decision procedure for linear real and rational arithmetic}
\label{sec:lra}
...
\asection{\tacticdef{lia}: a tactic for linear integer arithmetic }
\tacindex{lia}
\label{sec:lia}
...
\asection{\tacticdef{nia}: a proof procedure for non-linear integer arithmetic}
\tacindex{nia}
\label{sec:nia}
