...
\subsection{\vernacdef{Program Definition {\ident} := {\term}.
...
\begin{Variants}
\item {\vernacdef{Program Definition {\ident} {\tt :} {\term$_1$} :=
    {\term$_2$}.}}\\
...
\item {\vernacdef{Program Definition {\ident} {\binder$_1$}\ldots{\binder$_n$}
       {\tt :} \term$_1$ {\tt :=} {\term$_2$}.}}\\
...
\subsection{\vernacdef{Program Fixpoint {\ident} {\params} {\tt \{order\}} : type := \term
...
\subsection{\vernacdef{Program Lemma {\ident} : type.
...
\begin{itemize}
\item {\vernacdef{Local Obligation Tactic := \tacexpr}, \vernacdef{Global Obligation Tactic := \tacexpr}}\comindex{Obligation Tactic}
...
\item {\vernacdef{Show Obligation Tactic}}\comindex{Show Obligation Tactic}
  Displays the current default tactic.
\item {\vernacdef{Obligations \zeroone{of \ident}}}\comindex{Obligations} Displays all remaining
  obligations.
\item {\vernacdef{Obligation num \zeroone{of \ident}}}\comindex{Obligation} Start the proof of
  obligation {\tt num}.
\item {\vernacdef{Next Obligation \zeroone{of \ident}}}\comindex{Next Obligation} Start the proof of the next
  unsolved obligation.
\item {\vernacdef{Solve Obligations \zeroone{of \ident} \zeroone{with
    \tacexpr}}}\comindex{Solve Obligations}
  Tries to solve
  each obligation of \ident using the given tactic or the default one.
\item {\vernacdef{Solve All Obligations \zeroone{with \tacexpr}}} Tries to solve
  each obligation of every program using the given tactic or the default
  one (useful for mutually recursive definitions).
\item {\vernacdef{Admit Obligations \zeroone{of \ident}}}\comindex{Admit Obligations}
  Admits all obligations (does not work with structurally recursive programs).
\item {\vernacdef{Preterm \zeroone{of \ident}}}\comindex{Preterm}
  Shows the term that will be fed to
  the kernel once the obligations are solved. Useful for debugging.
\item {\vernacdef{Set Transparent Obligations}}\optindex{Transparent Obligations}
...
\item {\vernacdef{Set Hide Obligations}}\optindex{Hide Obligations}
...
\item {\vernacdef{Set Shrink Obligations}}\optindex{Shrink Obligations}
