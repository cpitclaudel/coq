...
The tactic
\begin{quote}
{\ltacdef{tryif {\tacexpr}$_1$ then {\tacexpr}$_2$ else {\tacexpr}$_3$}}
\end{quote}
...
\begin{quote}
{\ltacdef{{\tt exactly\_once} {\tacexpr}}}
\end{quote}
{\tacexpr} is evaluated to $v$ which must be a tactic value.
The tactic value $v$ is
...
panel of tactics:
\begin{quote}
{\ltacdef{{\tt solve [} {\tacexpr}$_1$ {\tt |} $...$ {\tt |} {\tacexpr}$_n$ {\tt ]}}}
\end{quote}
{\tacexpr}$_i$ are evaluated to $v_i$ and $v_i$ must be tactic values,
...
The constant {\tacticdef{idtac}} is the identity tactic: it leaves any goal
unchanged but it appears in the proof script.

\variant {\tacticdef{idtac \nelist{\messagetoken}{}}}
...
The tactic {\tacticdef{fail}} is the always-failing tactic: it does not solve
...
\begin{Variants}
\item {\tacticdef{fail $n$}}\\ The number $n$ is the failure level. If no
...
\begin{quote}
{\ltacdef{{\tt fun} {\ident${}_1$} ... {\ident${}_n$} {\tt =>} {\tacexpr}}}
\end{quote}
...
pattern:
\begin{quote}
{\ltacdef{{\tt context} {\ident} {\tt [} {\cpattern} {\tt ]}}}
\end{quote}
...
\begin{quote}
{\ltacdef{{\tt appcontext} {\ident} {\tt [} {\cpattern} {\tt ]}}}
\end{quote}
This syntax is now deprecated, as {\tt context} behaves as intended. The former
behavior can be retrieved with the {\vernacdef{Set Tactic Compat Context}} command.
...
expressions:
\begin{quote}
{\ltacdef{{\tt context} {\ident} {\tt [} {\tacexpr} {\tt ]}}}
\end{quote}
...
The number of goals under focus can be recovered using the {\ltacdef{numgoals}}
...
From the outside ``{\ltacdef{abstract \tacexpr}}'' is the same as
...
unless the proof is ended with ``{\vernacdef{Qed exporting}}''.  In such
case the lemma is preserved.  The syntax
``{\vernacdef{Qed exporting \ident$_1$, \ldots, \ident$_n$}}''
...
\begin{Variants}
\item {\ltacdef{abstract {\tacexpr} using {\ident}}}.\\
...
\begin{quote}
{\vernacdef{Set Ltac Debug.}}
\end{quote}

\noindent and deactivated using the command

\begin{quote}
{\vernacdef{Unset Ltac Debug.}}
\end{quote}
