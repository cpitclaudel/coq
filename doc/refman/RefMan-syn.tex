...
\begin{quote}
\vernacdef{Print Grammar constr.}
\end{quote}

\variant

\comindex{Print Grammar pattern}
{\vernacdef{Print Grammar pattern.}}\\
...
\begin{quote}
\noindent\vernacdef{\texttt{Infix "{\symbolentry}" :=} {\qualid} {\tt (} \nelist{\em modifier}{,} {\tt )}.}
\end{quote}
...
\begin{quote}
\vernacdef{Unset Printing Notations.}
\end{quote}
To reactivate it, use the command
\begin{quote}
\vernacdef{Set Printing Notations.}
\end{quote}
...
\begin{quote}
\vernacdef{Locate "{\symbolentry}"}
\end{quote}
where symbol is any (composite) symbol. To locate
...
\item {\vernacdef{Local Open Scope {\scope}.}}

\item {\vernacdef{Local Close Scope {\scope}.}}

These variants are not exported to the modules that import the module
where they occur, even if outside a section.

\item {\vernacdef{Global Open Scope {\scope}.}}

\item {\vernacdef{Global Close Scope {\scope}.}}
...
\begin{quote}
\vernacdef{\texttt{Delimit Scope} {\scope} \texttt{with} {\ident}}
\end{quote}

To remove a delimiting key of a scope, use the command

\begin{quote}
\vernacdef{\texttt{Undelimit Scope} {\scope}}
\end{quote}
...
\begin{quote}
{\vernacdef{Arguments {\qualid} : clear scopes}}
\end{quote}
...
\begin{quote}
{\vernacdef{Bind Scope {\scope} \texttt{with} {\qualid}}}
\end{quote}
...
commands {\vernacdef{Print Scopes.}} or {\vernacdef{Print Scope {\scope}.}}.

\subsubsection{\scopedef{type\_scope}}

This includes infix {\tt *} for product types and infix {\tt +} for
sum types. It is delimited by key {\tt type}.

\subsubsection{\scopedef{nat\_scope}}
...
\subsubsection{\scopedef{N\_scope}}
...
\subsubsection{\scopedef{Z\_scope}}

This includes the standard arithmetical operators and relations on
type {\tt Z} (binary integer numbers). It is delimited by key {\tt Z} 
and comes with an interpretation for numerals as closed term of type {\tt Z}.

\subsubsection{\scopedef{positive\_scope}}
...
\subsubsection{\scopedef{Q\_scope}}
...
\subsubsection{\scopedef{Qc\_scope}}
...
\subsubsection{\scopedef{real\_scope}}
...
\subsubsection{\scopedef{char\_scope}}
...
\subsubsection{\vernacdef{Print Visibility\comindex{Print Visibility}}}
...
{\vernacdef{Print Visibility {\scope}}}\\
...
\subsubsection{\vernacdef{Print Scope {\scope}\comindex{Print Scope}}}
...
\subsubsection{\vernacdef{Print Scopes\comindex{Print Scopes}}}
...
\begin{quote}
{\vernacdef{\zeroone{{\tt Local}} \texttt{Notation} {\ident} \sequence{\ident} {\ident} \texttt{:=} {\term}
 \zeroone{{\tt (only parsing)}}~.}}
\end{quote}
...
A tactic notation {\vernacdef{Tactic Notation {\taclevel}
...
existing tacticals, use the command {\vernacdef{Print Grammar tactic.}}
