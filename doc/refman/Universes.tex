...
declared using the \vernacdef{Polymorphic \nterm{definition}} construct. There also exists an
option \vernacdef{Set Universe Polymorphism} which will implicitly prepend
it to any definition of the user. In that case, to make a definition
producing global universe constraints, one can use the
\vernacdef{Monomorphic \nterm{definition}} construct. Many other commands support the
...
\subsection{\vernacdef{Universe {\ident}.
  \comindex{Universe}
  \label{UniverseCmd}}}
...
\subsection{\vernacdef{Constraint {\ident} {\textit{ord}} {\ident}.
  \comindex{Constraint}
  \label{ConstraintCmd}}}
...
\subsection{\vernacdef{Unset Strict Universe Declaration.
