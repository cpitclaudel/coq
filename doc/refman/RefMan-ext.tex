...
\smallskip
\vernacdef{{\tt Record} {\ident} {\params} \texttt{:} {\sort}
  := {\ident$_0$} \verb+{+
...
\qquad {\tt
  {\ident$_0$} ({\ident$_1$}:{\term$_1$}) \ldots ({\ident$_n$}:{\term$_n$}).}
\end{quote}
...
\begin{Warnings}
\item \errindex{{\ident$_i$} cannot be defined.}
...
\begin{quote}
\vernacdef{Unset Printing Records.}
\optindex{Printing Records}
\end{quote}
For a given type, one can override this using either
\begin{quote}
{\vernacdef{Add Printing Record {\ident}.}}
\end{quote}
to get record syntax or
\begin{quote}
{\vernacdef{Add Printing Constructor {\ident}.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Set Printing Projections.}}
\end{quote}
...
{\vernacdef{Unset Printing Projections}}.
...
The option {\vernacdef{Set Primitive Projections}} turns on the use of primitive
...
can be changed using the {\vernacdef{Unset Printing Primitive Projection Parameters}}
command. Further compatibility printing can be deactivated thanks to the
{\vernacdef{Unset Printing Primitive Projection Compatibility}} command which governs the
...
\begin{quote}
{\vernacdef{Set Printing Wildcard.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Unset Printing Wildcard.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Test Printing Wildcard.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Test Printing Let for {\ident}.}}
\end{quote}
This tells if {\ident} belongs to the list.

\begin{quote}
{\vernacdef{Print Table Printing Let.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Add Printing If {\ident}.}}
\end{quote}
This adds {\ident} to the list of inductive types for which
pattern-matching is written using an {\tt if} expression.

\begin{quote}
{\vernacdef{Remove Printing If {\ident}.}}
\end{quote}
This removes {\ident} from this list.

\begin{quote}
{\vernacdef{Test Printing If for {\ident}.}}
\end{quote}
This tells if {\ident} belongs to the list.

\begin{quote}
{\vernacdef{Print Table Printing If.}}
\end{quote}
This prints the list of inductive types for which pattern-matching is
...
\begin{center}
   \vernacdef{Function {\ident} {\binder$_1$}\ldots{\binder$_n$}
...
\subsection{\vernacdef{Section {\ident}\comindex{Section}}}
...
\subsection{\vernacdef{End {\ident}
\comindex{End}}}
...
\begin{quote}
{\vernacdef{Arguments {\qualid} : clear implicits
\comindex{Arguments}}}
\end{quote}

\begin{Variants}
\item {\vernacdef{Global Arguments {\qualid} \nelist{\possiblybracketedident}{}
...
\item {\vernacdef{Local Arguments {\qualid} \nelist{\possiblybracketedident}{}
...
\begin{quote}
{\vernacdef{Arguments {\qualid} : default implicits
\comindex{Arguments}}}
\end{quote}
...
\begin{Variants}
\item {\vernacdef{Global Arguments {\qualid} : default implicits
\comindex{Global Arguments}}}

Tell to recompute the implicit arguments of {\qualid} after ending of
the current section if any.

\item {\vernacdef{Local Arguments {\qualid} : default implicits
\comindex{Local Arguments}}}
...
\begin{quote}
{\vernacdef{Set Implicit Arguments.}}
\end{quote}
Conversely, one may unset the mode by using {\vernacdef{Unset Implicit
...
\begin{quote}
{\vernacdef{Unset Strict Implicit.}}
\end{quote}
Conversely, use the command {\vernacdef{Set Strict Implicit}} to
...
\begin{quote}
{\vernacdef{Set Strongly Strict Implicit.}}
\end{quote}
Conversely, use the command {\vernacdef{Unset Strongly Strict Implicit}} to
...
\begin{quote}
{\vernacdef{Set Contextual Implicit.}}
\end{quote}
Conversely, use command {\vernacdef{Unset Contextual Implicit}} to
...
argument, use command  
\begin{quote}
{\vernacdef{Set Reversible Pattern Implicit.}}
\end{quote}
Conversely, use command {\vernacdef{Unset Reversible Pattern Implicit}} to
...
\begin{quote}
{\vernacdef{Set Maximal Implicit Insertion.}}
\end{quote}
...
default mode for maximal insertion, use command {\vernacdef{Unset Maximal
Implicit Insertion}}.
...
\begin{quote}
{\vernacdef{Print Implicit {\qualid}.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Unset Printing Implicit Defensive.}}
\end{quote}
Conversely, to force the display of non strict arguments, use command
\begin{quote}
{\vernacdef{Set Printing Implicit Defensive.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Set Parsing Explicit.}}
\end{quote}
...
using the command
\begin{quote}
{\vernacdef{Canonical Structure {\qualid}.}}
\end{quote}
...
\begin{Variants}
\item {\vernacdef{Canonical Structure {\ident} := {\term} : {\type}.}}\\
 {\vernacdef{Canonical Structure {\ident} := {\term}.}}\\
 {\vernacdef{Canonical Structure {\ident} : {\type} := {\term}.}}

These are equivalent to a regular definition of {\ident} followed by
the declaration 

{\vernacdef{Canonical Structure {\ident}}}.
\end{Variants}
...
\begin{Variants}
\item {\vernacdef{Implicit Type {\ident} : {\type}}}\\
...
\begin{quote}
{\tt Generalizable \argchoice{All,No} Variable(s)? ({\ident$_1$ \ident$_n$})?.}
\end{quote}

\begin{Variants}
\item {\vernacdef{Generalizable All Variables.}} All variables are candidate for
...
\item {\vernacdef{Generalizable No Variables.}} Disable implicit generalization
  entirely. This is the default behavior.

\item {\vernacdef{Generalizable Variable {\ident}.}}
\item {\vernacdef{Generalizable Variables {\ident$_1$ \ldots \ident$_n$}.}}
  Allow generalization of the given identifiers only. Calling this
  command multiple times adds to the allowed identifiers.

\item {\vernacdef{Global Generalizable All Variables.}} Allows to export the choice of
...
\comindex{Coercion}
\begin{quote}
{\vernacdef{Coercion {\qualid} : {\class$_1$} >-> {\class$_2$}.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Set Printing Universes}}
\end{quote}
...
\begin{quote}
{\vernacdef{Unset Printing Universes.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Print \zeroone{Sorted} Universes.}}
\end{quote}
...
\begin{quote}
{\vernacdef{Print \zeroone{Sorted} Universes {\str}.}}
\end{quote}
...
The command:
\begin{quote}
{\vernacdef{Set Printing Existential Instances}}
\end{quote}
...
variables, use
\begin{quote}
{\vernacdef{Unset Printing Existential Instances.}}
\end{quote}
