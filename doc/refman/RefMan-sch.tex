...
\begin{quote}
{\vernacdef{Scheme {\ident$_1$} := Induction for \ident'$_1$ Sort {\sort$_1$} \\
  with\\
...
\begin{Variants}
\item {\vernacdef{Scheme {\ident$_1$} := Minimality for \ident'$_1$ Sort {\sort$_1$} \\
    with\\
    \mbox{}\hspace{0.1cm} \dots\ \\
...
\item {\vernacdef{Scheme Equality for \ident$_1$\comindex{Scheme Equality}}}
...
\item {\vernacdef{Scheme Induction for \ident$_1$ Sort {\sort$_1$} \\
  with\\
  \mbox{}\hspace{0.1cm} \dots\\
...
It is possible to deactivate the automatic declaration of the induction
 ...
 {\vernacdef{Unset Elimination Schemes}} command. It may be
reactivated at any time with {\vernacdef{Set Elimination Schemes}}.
...
command {\vernacdef{Set Nonrecursive Elimination Schemes}}. It can be
deactivated again with {\vernacdef{Unset Nonrecursive Elimination Schemes}}.

In addition, the {\vernacdef{Set Case Analysis Schemes}} command enables the generation of
...
with respectively the commands {\vernacdef{Set Boolean Equality Schemes}} and
{\vernacdef{Set Decidable Equality Schemes}}.
...
\begin{quote}
{\vernacdef{Combined Scheme {\ident$_0$} from {\ident$_1$}, .., {\ident$_n$}}}
\end{quote}
where
...
\begin{quote}
{\vernacdef{Functional Scheme {\ident$_1$} := Induction for \ident'$_1$ Sort {\sort$_1$} \\
  with\\
  \mbox{}\hspace{0.1cm} \dots\ \\
        with {\ident$_m$} := Induction for {\ident'$_m$} Sort
        {\sort$_m$}}}
...
\begin{quote}
{\vernacdef{Derive Inversion {\ident} with forall
  \ident$_1$: \type$_1$ \dots\ \ident$_n$: \type$_n$, \nterm{I} \termarg$_1$ \dots\ \termarg$_n$ Sort \sort}}
\end{quote}

This command generates an inversion principle for the
\texttt{inversion \dots\ using} tactic.
\tacindex{inversion \dots\ using}
Let $I$ be an inductive predicate and \ident$_1$ \dots\ \ident$_n$ the variables
occurring in \termarg$_1$ \dots\ \termarg$_n$. This command generates and stocks the
inversion lemma for the sort \sort~ corresponding to the instance
$\forall$ \ident$_1$: \type$_1$ \dots\ \ident$_n$: \type$_n$, \nterm{I} \termarg$_1$ \dots\ \termarg$_n$ with the name {\ident} in the {\bf
global} environment. When applied, it is equivalent to having inverted
the instance with the tactic {\tt inversion}.

\begin{Variants}
\item \vernacdef{Derive Inversion\_clear {\ident} with forall
  \ident$_1$: \type$_1$ \dots\ \ident$_n$: \type$_n$, \nterm{I} \termarg$_1$ \dots\ \termarg$_n$ Sort \sort}\\
...
\item \vernacdef{Derive Dependent Inversion {\ident} with forall
  \ident$_1$: \type$_1$ \dots\ \ident$_n$: \type$_n$, \nterm{I} \termarg$_1$ \dots\ \termarg$_n$ Sort \sort}\\
  \comindex{Derive Dependent Inversion}
  When applied, it is equivalent to having
  inverted the instance with the tactic \texttt{dependent inversion}.
\item \vernacdef{Derive Dependent Inversion\_clear {\ident} with forall
  \ident$_1$: \type$_1$ \dots\ \ident$_n$: \type$_n$, \nterm{I} \termarg$_1$ \dots\ \termarg$_n$ Sort \sort}\\
