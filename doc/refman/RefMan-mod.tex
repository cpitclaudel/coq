...
\subsection{\vernacdef{Module {\ident}
\comindex{Module}}}
...
\item{\vernacdef{Module {\ident} {\modbindings}}}

  Starts an interactive functor with parameters given by {\modbindings}.

\item{\vernacdef{Module {\ident} \verb.:. {\modtype}}}

  Starts an interactive module specifying its module type. 

\item{\vernacdef{Module {\ident} {\modbindings} \verb.:. {\modtype}}}

  Starts an interactive functor with parameters given by
  {\modbindings}, and output module type {\modtype}.

\item{\vernacdef{Module {\ident} \verb.<:. {\modtype$_1$} \verb.<:. $\ldots$ \verb.<:.{ \modtype$_n$}}}

  Starts an interactive module satisfying each {\modtype$_i$}. 

\item{\vernacdef{Module {\ident} {\modbindings} \verb.<:. {\modtype$_1$} \verb.<:. $\ldots$ \verb.<:. {\modtype$_n$}}}

  Starts an interactive functor with parameters given by
  {\modbindings}. The output module type is verified against each
  module type {\modtype$_i$}.

\item{\vernacdef{Module \argchoice{Import,Export} {\ident}}}
...
\item {\vernacdef{Include {\module$_1$} \verb.<+. $\ldots$ \verb.<+. {\module$_n$}}}

is a shortcut for {\vernacdef{Include {\module$_1$}}}  $\ldots$ {\vernacdef{Include {\module$_n$}}}
\end{enumerate}
...
\subsection{\vernacdef{Module {\ident} := {\modexpr}
\comindex{Module}}}
...
\begin{Variants}
\item{\vernacdef{Module {\ident} {\modbindings} := {\modexpr}}}
...
\item{\vernacdef{Module {\ident} {\modbindings} := {\modexpr$_1$} \verb.<+. $\ldots$ \verb.<+. {\modexpr$_n$}}}

  is equivalent to an interactive module where each {\modexpr$_i$} are included.

\end{Variants}

\subsection{\vernacdef{Module Type {\ident}
...
\item{\vernacdef{Module Type {\ident} {\modbindings}}}
...
\item {\vernacdef{{\assumptionkeyword} Inline {\assums} }}
...
\subsection{\vernacdef{Module Type {\ident} := {\modtype}}}

Defines a module type {\ident} equal to {\modtype}.

\begin{Variants}
\item {\vernacdef{Module Type {\ident} {\modbindings} := {\modtype}}}

  Defines a functor type {\ident} specifying functors taking arguments
  {\modbindings} and returning {\modtype}.

\item{\vernacdef{Module Type {\ident} {\modbindings} := {\modtype$_1$} \verb.<+. $\ldots$ \verb.<+. {\modtype$_n$}}}

  is equivalent to an interactive module type were each {\modtype$_i$} are included.

\end{Variants}

\subsection{\vernacdef{Declare Module {\ident} : {\modtype}
...
\item{\vernacdef{Declare Module {\ident} {\modbindings} \verb.:. {\modtype}}}
...
\subsection{\vernacdef{Import {\qualid}
\comindex{Import}
\label{Import}}}
...
\begin{Variants}
\item{\vernacdef{Export {\qualid}}}\comindex{Export}
...
\subsection{\vernacdef{Print Module {\ident}
\comindex{Print Module}}}

Prints the module type and (optionally) the body of the module {\ident}.

\subsection{\vernacdef{Print Module Type {\ident}
\comindex{Print Module Type}}}

Prints the module type corresponding to {\ident}.

\subsection{\vernacdef{Locate Module {\qualid}
